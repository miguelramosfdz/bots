...
module.exports = function createApp (conf) {
  ...
  const botOpts = {
    ...
    get: co(function* ({ link }) {
      const resp = yield get({ path: `/object/${link}` })
      return interpretObjectWrapper(resp)
    }),
    ...
    send: co(function* ({ userId, object, other={} }) {
      ...
      const resp = yield post({
        path: '/message',
        data
      })

      return interpretMessageWrapper(resp)
    }),

    /**
     * @return {Promise}
     */
    seal: co(function* ({ link }) {
  ...
  app.post('/', bigJsonParser(), coExpress(function* (req, res) {
    ...
    switch (event) {
      case 'message':
        yield bot.receive(interpretMessageWrapper(data))
        break
      case 'newversion':
        yield bot.seals.onnewversion(data)
        break
      case 'readseal':
